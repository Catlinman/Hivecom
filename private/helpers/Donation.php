...
class Donation {
    public static function dbconnect() {
        ...
        if (mysqli_connect_errno()) {
            error_log(date("Y-m-d H:i:s ") . "Donation/dbconnect:" . mysqli_connect_error() . ".\n", 3, SITE_LOG);

        } else {
            // Hivecom database should be selected.
            mysqli_select_db($dbconnection, "hivecom")
                or error_log(date("Y-m-d H:i:s ") . "Donation/dbconnect:" . mysqli_error($dbconnection) . ".\n", 3, SITE_LOG);
    ...
    public static function retrieveLatest() {
        $dbconnection = Donation::dbconnect(); // Make the database connection in not already present.

        // Make the main query.
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "SELECT * FROM donations ORDER BY `date_start` DESC LIMIT 1;")
                or error_log(date("Y-m-d H:i:s ") . "Donation/retrieveLatest: " . mysqli_error($dbconnection) . ".\n", 3, SITE_LOG);
