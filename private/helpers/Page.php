...
class HivecomPage {
    ...
    */
    public static function prepare($title, $subtitle, $author, $opening_md, $content_md, $is_news, $is_sticky) {
        ...
        return array(
            ...
            HivecomPage::SQL_SUBTITLE_INDEX     => $subtitle,
            HivecomPage::SQL_OPENING_MD_INDEX   => $opening_md,
    ...
    public static function create($data) {
        ...
        if ($dbconnection) {
            ...
            try {
				...
				$access_id = HivecomUtility::slug(mysqli_real_escape_string($dbconnection, $data[HivecomPage::SQL_TITLE_INDEX]));

				// Check that the access identifier is unique. Otherwise prepend the unique identifier.
				if (HivecomPage::retrieveViaAccess($access_id)) {
					$access_id = HivecomUtility::slug($unique_id . " " . mysqli_real_escape_string($dbconnection, $data[HivecomPage::SQL_TITLE_INDEX]));
				}
                ...
                $opening_html = mysqli_real_escape_string(
                    $dbconnection,
                    HivecomUtility::parsedown($data[HivecomPage::SQL_OPENING_MD_INDEX])
                );
    ...
    public static function remove($unique_id) {
        ...
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "DELETE FROM `pages` WHERE `unique_id` = $unique_id;")
                or error_log(date("Y-m-d H:i:s ") . "HivecomPage/remove: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
    ...
    public static function assignAccess($unique_id, $access_id) {
        ...
        if ($dbconnection) {
            ...
            $result = mysqli_query(
				...
				WHERE `unique_id` = '$unique_id'
				;"
            ) or error_log(date("Y-m-d H:i:s ") . "HivecomPage/assignAccess: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
    ...
    */
    public static function resetAccess($unique_id) {
        $page = HivecomPage::retrieve($unique_id);
        ...
        $access_id = HivecomUtility::slug($unique_id . " " . mysqli_real_escape_string($data[HivecomPage::SQL_TITLE_INDEX]));
        ...
        if ($dbconnection) {
            ...
            $result = mysqli_query(
				...
				WHERE `unique_id` = '$unique_id'
				;"
            ) or error_log(date("Y-m-d H:i:s ") . "HivecomPage/resetAccess: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
        }
    }
    ...
    public static function retrieveViaAccess($access_id) {
        ...
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "SELECT * FROM `pages` WHERE `access_id` = '$access_id';")
                or error_log(date("Y-m-d H:i:s ") . "HivecomPage/retrieveViaAccess: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
    ...
    public static function retrieveSticky() {
        ...
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "SELECT * FROM `pages` WHERE `is_sticky` = 1 ORDER BY `date_create` DESC LIMIT 1;")
                or error_log(date("Y-m-d H:i:s ") . "HivecomPage/retrieveSticky: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
    ...
    public static function retrieveNews($count) {
        ...
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "SELECT * FROM `pages` WHERE `is_news` = 1 ORDER BY `date_create` DESC LIMIT $count;")
                or error_log(date("Y-m-d H:i:s ") . "HivecomPage/retrieveNews: " . mysqli_error($dbconnection) . "\n", 3, SITE_LOG);
    ...
    public static function count() {
        ...
        if ($dbconnection) {
            $result = mysqli_query($dbconnection, "SELECT COUNT(`page_id`) FROM `pages`;")
                or error_log(date("Y-m-d H:i:s ") . "HivecomPage/count: " . mysqli_error($dbconnection) . ".\n", 3, SITE_LOG);
